        ...
        if st.button("Oblicz prawdopodobieństwo", type="primary"):
            if not query:
                ...
                def extract_advanced_features(query):
                    ...
                    # 1. INTENT
                    info_patterns = r'jak|co to|dlaczego|ile|kiedy|czy|znaczenie|definicja|powody|objawy|zasady|historia|opis|przepis|poradnik|instrukcja'
                    ...
                    # 2. FUNNEL STAGE
                    features['stage_tofu'] = 1 if re.search(r'jak|co|kto|gdzie|dlaczego|czym|znaczenie', q) else 0
                ...
                with col2:
                    if prediction < 30:
                        ...
                        msg = "🔥 Wysokie prawdopodobieństwo! Fraza spełnia kryteria raportu Senuto."
                    st.progress(prediction / 100)
                    st.caption(msg)
                    
                # Feature explanation (simplified)
                with st.expander("Analiza cech frazy (Scoring)"):
                    st.write("Model przeanalizował frazę pod kątem wytycznych z raportu:")
                    f = features
                    groups = {
                        "📁 Struktura": ["word_count", "char_count", "is_long_tail", "starts_with_q"],
                        "🧠 Intencja": ["intent_info", "intent_trans", "intent_local", "intent_nav"],
